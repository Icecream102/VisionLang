#!/usr/bin/env bash
# Run the complete experiment on AutoDL public COCO2017/ImageNet100.
# Stages run in sequence: MAE pre-training, linear probing, then three
# image-text retrieval initialisation/fine-tuning comparisons.
set -euo pipefail
...
# Leave empty for all 118,287 COCO train images. Set it only for a pilot.
train_limit="${TRAIN_LIMIT_IMAGES:-}"

cd "$project_root"

"$python_bin" -m examples.representation_learning.mae_pretrain \
  --data-root "$imagenet_root" --val-fraction 0.1 \
  --output-dir outputs/full_imagenet100_mae_tiny_r75 \
...
"$python_bin" -m examples.representation_learning.mae_linear_probe \
  ...
  --checkpoint outputs/full_imagenet100_mae_tiny_r75/last.pt \
  --output outputs/full_imagenet100_mae_tiny_r75/linear_probe.json \
