...
class MaskedAutoEncoder(nn.Module):
    ...
    def forward(self, x: Tensor, return_reconstruction: bool = False) -> MAEOutput:
        """
        Args:
            x (Tensor): input tensor with shape bsz x channels x height x width
            return_reconstruction: also run the reconstruction decoder in eval mode.
        Returns:
        ...
        """
        # Reconstruction validation should retain eval-mode encoder/decoder behavior
        # while still applying MAE's random patch masking.
        embedding_out = self.embeddings(
            x, apply_patch_drop=self.training or return_reconstruction
        )
        encoder_out = self.encoder(embedding_out.embeddings)
        if not self.training and not return_reconstruction:
