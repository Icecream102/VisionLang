...
class PatchEmbeddings(nn.Module):
    ...
    def forward(
        ...
        apply_patch_drop: Optional[bool] = None,
    ) -> PatchEmbeddingsOutput:
        """Embed image patches.

        ``apply_patch_drop`` overrides the module training flag for random patch
        dropping. It is useful when a masked-reconstruction validation pass should
        keep all other layers in evaluation mode.
        """
        ...
        ids_restore = None
        if (
            self.training if apply_patch_drop is None else apply_patch_drop
        ) and self.patch_drop_rate is not None:
